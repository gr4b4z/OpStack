...
### 4.7.2 Allocate IPs from public network to controller nodes: 
*  opstack-admin
```bash
crowbar network allocate_ip "default" opstack-node01.ocean.local "public" "host" 
crowbar network enable_interface "default" opstack-node01.ocean.local "public" 
 
crowbar network allocate_ip "default" opstack-node02.ocean.local "public" "host" 
crowbar network enable_interface "default" opstack-node02.ocean.local "public" 
```
????Should be also on node03-06?
...
partprobe 
mkfs.xfs /dev/rbd1p1
#Restart RBD map service on **opstack-node02**
systemctl restart rbdmap 
```
3.  Open SUSE OpenStack Cloud admin web portal <http://ADMIN_NODE_IP_ON_ADMIN_NETWORK> and go to Barclamps à OpenStack 
